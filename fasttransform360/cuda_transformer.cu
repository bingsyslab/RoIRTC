...
__global__ void CUDATransformKernelNPL(uint8_t* transformed_data,
                                       int w,
                                       float rw,
                                       float rh,
                                       float ou,
  ...
  int i = threadIdx.x + blockIdx.x * blockDim.x;
  if (i < n) {
    float u = __fmaf_rn(__int2float_rn(i % w) + 0.5, rw, -ou),
          v = __fmaf_rn(__int2float_rn(i / w) + 0.5, rh, -ov);
    float fu = __int2float_rn(__float2int_rd(u + 1.0)),
          fv = __int2float_rn(__float2int_rd(v + 1.0));
    fu = __fmaf_rn(__fmaf_rn(-2.0, fu, 1.0), ou, fu);
    fv = __fmaf_rn(__fmaf_rn(-2.0, fv, 1.0), ov, fv);
    u = __fdiv_rn(u, fu);
    v = __fdiv_rn(v, fv);
    u = tanf(0.7853981633974483 * u);
    v = tanf(0.7853981633974483 * v);
    transformed_data[i] = tex2D(tex_ref,
        __fmaf_rn(fu, u, ou),
        __fmaf_rn(fv, v, ov));
  }
...
__global__ void CUDATransformKernelAPL(uint8_t* transformed_data,
                                       int w,
                                       float rw,
                                       float rh,
                                       float dx,
  ...
  if (i < n) {
    float u = (__int2float_rn(i % w) + 0.5) * rw,
          v = (__int2float_rn(i / w) + 0.5) * rh;
    // BRANCH
    int flx = __float2int_rd(__fdiv_rn(u, __fmaf_rn(2.0, hlx, dx))),
        fly = __float2int_rd(__fdiv_rn(v, __fmaf_rn(2.0, hly, dy))),
        fdx = __float2int_rd(__fdiv_rn(dx, u)),
        fdy = __float2int_rd(__fdiv_rn(dy, v)); // BRANCH */
    /* NON-BRANCH
    int fdx = __float2int_rd(__fdiv_rn(dx, u + dx) + 0.5),
        flx = __float2int_rd(__fdiv_rn(u, __fmaf_rn(2.0, hlx, u + dx)) + 0.5);
    float andx = __fmaf_rn(fdx, flx, 1.0 - fdx - flx);
    int fdy = __float2int_rd(__fdiv_rn(dy, v + dy) + 0.5),
        fly = __float2int_rd(__fdiv_rn(v, __fmaf_rn(2.0, hly, v + dy)) + 0.5);
    float andy = __fmaf_rn(fdy, fly, 1.0 - fdy - fly); // NON-BRANCH */
    // BRANCH
    if (flx == 0 && fdx == 0) {
      u = __fmaf_rn(__frcp_rn(hlx), u - dx, -1.0);
      u = tanf(0.7853981633974483 * u);
      u = __fmaf_rn(hlx, u + 1.0, dx);
    }
    if (fly == 0 && fdy == 0) {
      v = __fmaf_rn(__frcp_rn(hly), v - dy, -1.0);
      v = tanf(0.7853981633974483 * v);
      v = __fmaf_rn(hly, v + 1.0, dy);
    } // BRANCH */
    /* NON-BRANCH
    float ou = __fmaf_rn(__frcp_rn(hlx), u - dx, -1.0);
    ou = tanf(0.7853981633974483 * ou);
    ou = __fmaf_rn(hlx, ou + 1.0, dx);
    u = __fmaf_rn(andx, ou, -__fmaf_rn(andx, u, -u));
    float ov = __fmaf_rn(__frcp_rn(hly), v - dy, -1.0);
    ov = tanf(0.7853981633974483 * ov);
    ov = __fmaf_rn(hly, ov + 1.0, dy);
    v = __fmaf_rn(andy, ov, -__fmaf_rn(andy, v, -v)); // NON-BRANCH */
    transformed_data[i] = tex2D(tex_ref, u, v);
  }
}
...
int CUDATransformWrapper(const uint8_t* data,
  ...
  switch (flag) {
    ...
    case NAIVE_PLANE:
    {
      ...
      uint8_t* cuda_data = NULL;
      cudaMalloc((void**)&cuda_data, n);
      int nt = min(max(target_width, target_height), THREAD_LIMIT);
      CUDATransformKernelNPL<<<n / nt, nt>>>(cuda_data,
                                             target_width,
                                             rw, rh,
                                             yaw, pitch, n);
      cudaDeviceSynchronize();
    ...
    case ADVANCED_PLANE:
    {
      ...
      cudaMalloc((void**)&cuda_data, n);
      int nt = min(max(target_width, target_height), THREAD_LIMIT);
      CUDATransformKernelAPL<<<n / nt, nt>>>(cuda_data,
                                             target_width,
                                             rw, rh,
                                             yaw, pitch,
                                             x, y, n);
